...
async def test_create_and_read(async_client):
    ...
    assert response_obj["title"] == "테스트 작업"      # 응답 JSON에 title이 잘 들어갔는지 확인
...
async def test_done_flag(async_client):
    ...
    assert response.status_code == status.HTTP_200_OK  # 완료 처리 요청이 성공했는지 확인
    ...
    # --------------------------------------------------------
    response = await async_client.put("/tasks/1/done")
    assert response.status_code == status.HTTP_400_BAD_REQUEST  # 중복 완료 요청 → 잘못된 요청 처리 확인
    ...
    assert response.status_code == status.HTTP_200_OK  # 정상적으로 완료 해제되었는지 확인
...
# ------------------------------------------------------------
# [테스트 함수] 마감일(due_date)에 잘못된 날짜가 들어간 경우
# - 사용자가 title과 함께 due_date를 전송했을 때,
#   날짜 형식이 잘못되면 서버가 오류(422)를 반환하는지 확인
# - 예: {"title": "테스트 작업", "due_date": "2024-12-32"}  ← 유효하지 않은 날짜
# ------------------------------------------------------------
@pytest.mark.asyncio
